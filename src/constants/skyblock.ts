...
export const COSMETIC_SKILLS = ['carpentry', 'runecrafting', 'social2'] as const;
...
export const SKILL_XP_TOTAL: number[] = [...SKILL_XP_PAST_50];
for (let i = 1; i < SKILL_XP_PAST_50.length; ++i) {
	SKILL_XP_TOTAL[i] = SKILL_XP_TOTAL[i - 1]! + SKILL_XP_PAST_50[i]!;
}
...
export const DUNGEON_XP_TOTAL: number[] = [...DUNGEON_XP];
for (let i = 1; i < DUNGEON_XP.length; ++i) {
	DUNGEON_XP_TOTAL[i] = DUNGEON_XP_TOTAL[i - 1]! + DUNGEON_XP_TOTAL[i]!;
}
...
export const SLAYER_XP = [0, 5, 15, 200, 1_000, 5_000, 20_000, 100_000, 400_000, 1_000_000] as const;
