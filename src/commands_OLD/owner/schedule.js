...
module.exports = class ScheduleCommand extends Command {
	...
	async run(message, args) { // eslint-disable-line no-unused-vars
		...
		if (flags.some(flag => [ 'l', 'list' ].includes(flag))) return message.reply({
			content: cronJobs.cache.keyArray().join('\n'),
			code: 'prolog',
			split: { char: '\n' },
		});
		...
		await message.reply({
			content: `\`${INPUT_STRING}\` scheduled for \`${LOCALE_DATE_STRING}\`, total amount of cron jobs: ${cronJobs.size}`,
