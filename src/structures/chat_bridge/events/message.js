...
module.exports = async (chatBridge, message) => {
	...
	switch (message.type) {
		...
		case WHISPER: {
			if (!chatBridge.enabled || message.me) return;
			if (message.author.player?.guildId !== chatBridge.guild.guildId) return logger.info(`[MESSAGE]: ignored message from '${message.author}': ${message.content}`); // ignore messages from non guild players
