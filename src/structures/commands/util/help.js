			...
			try {
				messages.push(await msg.direct(stripIndents`
					${oneLine`
						...
						use ${Command.usage('command', msg.guild?.commandPrefix ?? null, this.client.user)}.
						For example, ${Command.usage('prefix', msg.guild?.commandPrefix ?? null, this.client.user)}.
					`}
					...
					${groups.filter(grp => grp.commands.some(cmd => !cmd.hidden && (showAll || cmd.isUsable(msg)))).map(grp => stripIndents`
						__${grp.name}__
						${grp.commands.filter(cmd => !cmd.hidden && (showAll || cmd.isUsable(msg))).map(cmd => `**${cmd.name}:** ${cmd.description}${cmd.nsfw ? ' (NSFW)' : ''}`).join('\n')}
					`).join('\n\n')}
				`, { split: true }));
				if (msg.channel.type !== 'dm') messages.push(await msg.reply('Sent you a DM with information.'));
			} catch(err) {
