...
const { LEVELING_XP, SKILL_XP_PAST_50, SKILLS_CAP, RUNECRAFTING_XP, DUNGEON_XP, SLAYER_XP, SKILLS, COSMETIC_SKILLS, SLAYERS, DUNGEON_TYPES, DUNGEON_CLASSES } = require('../../src/constants/skyblock');
const { SKILL_EXPONENTS, SKILL_DIVIDER, SLAYER_DIVIDER, DUNGEON_MAX_POINTS } = require('../../src/constants/weight');
const { escapeIgn, getHypixelClient } = require('../../src/functions/util');
const { getRolesToPurge } = require('../../src/functions/database');
const logger = require('../../src/functions/logger');
const mojang = require('../../src/api/mojang');
	...
	class Player extends Model {
		...
		getWeight(offset = '') {
			...
			for (const skill of SKILLS) {
				const level = this.getSkillLevel(skill, offset).nonFlooredLevel;
			...
			for (const slayer of SLAYERS) {
				...
				weight += experience <= 1000000 ? experience / SLAYER_DIVIDER[slayer] : 1000000 / SLAYER_DIVIDER[slayer] + Math.pow((experience - 1000000) / (SLAYER_DIVIDER[slayer] * 1.5), 0.942);
			}

			const maxXp = Object.values(DUNGEON_XP).reduce((acc, xp) => acc + xp, 0);

			for (const type of [ ...DUNGEON_TYPES, ...DUNGEON_CLASSES ]) {
				const maxPoints = DUNGEON_MAX_POINTS[type];
				const xp = this[`${type}Xp${offset}`];

				weight += Math.pow(this.getSkillLevel(type, offset).nonFlooredLevel * 10, 3) * (maxPoints / 100000) / 1250;
				if (xp > maxXp) overflow += Math.pow((xp - maxXp) / (4 * maxXp / maxPoints), 0.968);
			}
