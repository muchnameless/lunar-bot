...
const { autocorrect, findNearestCommandsChannel, checkBotPermissions, getHypixelClient } = require('../../src/functions/util');
const { findMemberByTag } = require('../../src/functions/database');
const { Y_EMOJI, X_EMOJI, CLOWN } = require('../../src/constants/emojiCharacters');
const { offsetFlags } = require('../../src/constants/database');
const hypixel = require('../../src/api/hypixel');
	...
	class HypixelGuild extends Model {
		...
		async update() {
			...
			await Promise.all(membersJoined.map(async hypixelGuildMember => {
				const dbEntry = await this.db.Player.findByPk(hypixelGuildMember.uuid);
